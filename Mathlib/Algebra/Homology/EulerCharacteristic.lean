...
noncomputable def boundedEulerChar (C : ChainComplex (ModuleCat R) ℤ)
    (indices : Finset ℤ) : ℤ :=
  indices.sum (fun i => (-1 : ℤ) ^ i.natAbs * Module.finrank R (C.X i))
...
noncomputable def homologyBoundedEulerChar (C : ChainComplex (ModuleCat R) ℤ)
    ...
    (indices : Finset ℤ) : ℤ :=
  indices.sum (fun i => (-1 : ℤ) ^ i.natAbs * Module.finrank R (C.homology i))
...
noncomputable def eulerChar (C : ChainComplex (ModuleCat R) ℤ) : ℤ :=
  ∑' i : ℤ, (-1 : ℤ) ^ i.natAbs * Module.finrank R (C.X i)

/-- The homological Euler characteristic as an infinite sum. -/
noncomputable def homologyEulerChar (C : ChainComplex (ModuleCat R) ℤ)
    [∀ i : ℤ, (C.sc i).HasHomology] : ℤ :=
  ∑' i : ℤ, (-1 : ℤ) ^ i.natAbs * Module.finrank R (C.homology i)
...
theorem eulerChar_eq_boundedEulerChar (C : ChainComplex (ModuleCat R) ℤ)
    ...
    eulerChar C = boundedEulerChar C indices := by
  simp only [eulerChar, boundedEulerChar]
  have h_support : ∀ i ∉ indices, (-1 : ℤ) ^ i.natAbs * Module.finrank R (C.X i) = 0 := by
    intro i hi
    simp [h_zero i hi]
...
characteristic equals the bounded one. -/
theorem homologyEulerChar_eq_homologyBoundedEulerChar (C : ChainComplex (ModuleCat R) ℤ)
    [∀ i : ℤ, (C.sc i).HasHomology]
    (indices : Finset ℤ)
    (h_zero : ∀ i ∉ indices, Module.finrank R (C.homology i) = 0) :
    homologyEulerChar C = homologyBoundedEulerChar C indices := by
  simp only [homologyEulerChar, homologyBoundedEulerChar]
  have h_support : ∀ i ∉ indices,
      (-1 : ℤ) ^ i.natAbs * Module.finrank R (C.homology i) = 0 := by
    intro i hi
    simp [h_zero i hi]
  rw [tsum_eq_sum h_support]
